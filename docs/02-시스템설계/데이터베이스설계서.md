...
| `collection_method` | VARCHAR(20) | NOT NULL | `'context7'` | 수집 방식 (context7, crawling) |
| `context7_library_id` | VARCHAR(255) | NULL | NULL | Context7 라이브러리 ID (예: /openai/api). context7 수집 시 사용 |
...
| CK | `ck_api_providers_collection_method` | `collection_method` | `IN ('context7', 'crawling')` |
...
| `idx_api_providers_collection_method` | `collection_method` | B-tree | 수집 방식별 필터링 |
...
Context7 API(주력) 또는 직접 크롤링(보조)으로 수집되고 벡터 인덱싱(Indexing)이 완료된 공식 문서의 메타데이터를 저장하는 테이블. 실제 문서 본문은 벡터 DB에 저장되며, 이 테이블은 관리용 메타 정보만 보유한다.
...
| 수집 방식 | API 문서 수집 채널 | VARCHAR(20) | context7, crawling | context7: Context7 API 주력, crawling: 직접 크롤링 보조 |
| Context7 라이브러리 ID | Context7 내 라이브러리 식별자 | VARCHAR(255) | 경로 형식 (예: /openai/api) | resolve-library-id로 조회 |
...
| 카테고리 | 초기 등록 API 제공자 (예정) | 수집 방식 |
|----------|---------------------------|-----------|
| llm | OpenAI, Anthropic, Google Gemini, Cohere, Mistral | context7 |
| payment | Stripe, Paddle, LemonSqueezy | context7 |
| payment | Toss Payments | crawling (한국 API, Context7 미지원) |
| auth | Auth0, Clerk, Firebase Auth, Supabase Auth | context7 |
| messaging | Twilio, SendGrid, Resend, Mailgun | context7 |
| vector_db | Pinecone, Qdrant, Weaviate, Chroma | context7 |
