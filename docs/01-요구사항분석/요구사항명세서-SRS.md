...
|-------|--------|------|----------|---------|------|
| FR-001 | 회원가입 | GitHub OAuth 기반 회원가입 및 인증 | P1 필수 | UC-001 | 초안 |
| FR-002 | 로그인/로그아웃 | GitHub OAuth 로그인 지원 | P1 필수 | UC-002 | 초안 |
...
| **FR-ID** | FR-001 |
| **기능명** | 회원가입 |
| **설명** | 사용자가 GitHub OAuth를 통해 시스템에 회원으로 등록할 수 있다. GitHub 계정 인증을 통해 본인 확인을 수행한다. |
| **우선순위** | P1 필수 |
...
|--------|------|-----------|------------------|
| GitHub OAuth Token | String | 필수 | GitHub OAuth Provider로부터 수신 |
| 서비스 이용약관 동의 | Boolean | 필수 | true (최초 로그인 시) |
| 개인정보 처리방침 동의 | Boolean | 필수 | true (최초 로그인 시) |
| 마케팅 수신 동의 | Boolean | 선택 | - |

#### 처리 로직

1. GitHub OAuth 인증 콜백 수신
2. GitHub API를 통해 사용자 프로필 정보 조회 (이메일, 이름, avatar 등)
3. GitHub ID 기반 기존 회원 확인
4. 신규 사용자인 경우 자동 회원가입 (사용자 레코드 생성, 상태: `ACTIVE`)
5. 기존 사용자인 경우 로그인 처리
6. Access Token (JWT) 및 Refresh Token 발급
7. 로그인 이력 기록
...
| **FR-ID** | FR-002 |
| **기능명** | 로그인/로그아웃 |
| **설명** | 사용자가 GitHub OAuth를 통해 시스템에 인증하고, 인증 세션을 종료할 수 있다. |
| **우선순위** | P1 필수 |
...
| 필드명 | 타입 | 필수 여부 | 유효성 검증 규칙 |
|--------|------|-----------|------------------|
| GitHub OAuth Code | String | 필수 | GitHub OAuth 콜백으로부터 수신 |

#### 처리 로직

**GitHub OAuth 로그인:**

1. GitHub OAuth 인증 콜백에서 authorization code 수신
2. Authorization code를 GitHub Access Token으로 교환
3. GitHub API를 통해 사용자 식별 정보 조회
4. GitHub ID로 기존 사용자 조회 (없을 경우 자동 회원가입)
5. 계정 상태 확인 (ACTIVE 여부)
6. Access Token (JWT) 발급 -- 유효기간: [placeholder: e.g., 1시간]
7. Refresh Token 발급 -- 유효기간: [placeholder: e.g., 14일]
8. 로그인 이력 기록 (IP, User-Agent, 시간)
...
| GitHub 인증 실패 | GitHub OAuth 인증 실패 안내 | 401 Unauthorized |
| 계정 비활성 | 계정 비활성 상태 안내 | 403 Forbidden |
...
|---------|--------|--------|--------|------|-----------|
| NTF-01 | 회원가입 완료 | O | - | - | In-App 활성 |
| NTF-02 | 새 댓글/답글 | O | O | O | In-App 활성 |
| NTF-03 | 게시글 좋아요 | O | - | O | In-App 활성 |
| NTF-04 | 시스템 공지 | O | O | O | 전체 활성 |
| [placeholder] | [placeholder] | [placeholder] | [placeholder] | [placeholder] | [placeholder] |
...
| OAuth 인증 | GitHub OAuth 전용 | GitHub OAuth 2.0 PKCE 권장 |
...
| 화면 ID | 화면명 | 설명 | 관련 FR |
|---------|--------|------|---------|
| SCR-01 | 로그인 | GitHub OAuth 로그인 | FR-002 |
| SCR-02 | 회원가입 | GitHub OAuth 회원가입 (자동) | FR-001 |
